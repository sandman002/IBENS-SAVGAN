...
class deconv_block(nn.Module):
	...
	def __init__(self, in_dim, out_dim, kernel_size=1, interpolation='nearest', scale_factor=2, padding='same'):
		...
		self.up = nn.Upsample(scale_factor = scale_factor, mode=interpolation)
...
class Generator(nn.Module):
	...
	def __init__(self, batch_size, image_size=64, z_dim=100, conv_dim=64):
		...
		mult = 2 ** repeat_num # 8
		# deconv0 = deconv_block(z_dim, conv_dim*mult, kernel_size = (2,4,4), scale_factor = (1,2,2))
		# layer1.append(deconv0)
		layer1.append(spectral_norm(nn.ConvTranspose3d(z_dim, conv_dim * mult, (2,4,4))))
		layer1.append(nn.BatchNorm3d(conv_dim * mult))
		layer1.append(nn.ReLU())

		curr_dim = conv_dim * mult

		deconv1 = deconv_block(curr_dim, int(curr_dim / 2), kernel_size = 3, scale_factor = 2, padding=1)
		layer2.append(deconv1)
		...
		deconv2 = deconv_block(curr_dim, int(curr_dim / 2), kernel_size = 3, scale_factor = 2, padding=1)
		layer3.append(deconv2)
		...
		for _ in range(3):
			layer3.append(ResidualBlock(int(curr_dim / 2)))


		if self.imsize >= 64:
			layer4 = []
			curr_dim = int(curr_dim / 2)
			deconv3 = deconv_block(curr_dim, int(curr_dim / 2), kernel_size = 3, scale_factor = 2, padding=1)
			layer4.append(deconv3)
		...
		deconv4 = deconv_block(curr_dim, 1, kernel_size = 3, scale_factor = 2, padding=1)
		last.append(deconv4)
